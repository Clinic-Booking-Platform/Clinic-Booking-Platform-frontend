...
import type { UserStatusFilter } from '@/types/user.types'
...
export const UserFilter: React.FC<UserFilterProps> = ({
  ...
  const statusOptions: {
    ...
    { value: 'deleted', label: 'Tài khoản bị khóa', icon: Lock },
  ]
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-border/80 bg-card p-3 sm:p-4 shadow-xs">
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        {/* 1. Ô tìm kiếm đa năng (Tên, Email, Số điện thoại) */}
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Tìm kiếm theo Họ tên, Email, Số điện thoại bệnh nhân..."
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-t border-border/40 pt-2.5">
        ...
        {totalItems !== undefined && (
          <div className="text-xs text-muted-foreground font-medium pr-1">
            Tổng cộng: <strong className="text-foreground">{totalItems}</strong> người dùng
          </div>
        )}
      </div>
    </div>
