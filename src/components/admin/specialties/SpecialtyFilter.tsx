...
import { Search, X, RefreshCw, CheckCircle2, Trash2, Layers } from 'lucide-react'
import { Button } from '@/components/ui/button'
import type { SpecialtyStatusFilter } from '@/types/specialty.types'

interface SpecialtyFilterProps {
  search: string
  onSearchChange: (value: string) => void
  status: SpecialtyStatusFilter
  onStatusChange: (status: SpecialtyStatusFilter) => void
  onRefresh: () => void
  isLoading?: boolean
  totalItems?: number
}
...
export const SpecialtyFilter: React.FC<SpecialtyFilterProps> = ({
  ...
  const statusOptions: {
    ...
    { value: 'deleted', label: 'Thùng rác / Đã xóa', icon: Trash2 },
  ]
